...
from util import SDFDataset, extract_fields, listdir_by_time, mape_loss, mean_relative_l2, save_mesh, sort_files_with_number_at_end, Image, peak_signal_noise_ratio
import wandb
...
def get_args():
    ...
    parser.add_argument("baseline_checkpoint_dir", type=str, help="relative checkpoint directory of the trained lora or fully finetuned model (output of train_lora.py)")
...
if __name__ == "__main__":
    ...
    config = json.load(open(f"{args.baseline_checkpoint_dir}/config.json"))
    device = torch.device(f"cuda:{args.device}")
    timestamp = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d_%H-%M-%S')
    eval_dir = f"eval/{args.baseline_checkpoint_dir}/eval_{timestamp}/"
    os.makedirs(eval_dir)

    # determine whether to use lora or fully finetuned params
    checkpoint_files = list_files_with_extension(args.baseline_checkpoint_dir, ".pt")
    ...
    if args.video:
        ...
        reference_paths = sort_files_with_number_at_end(os.listdir(args.baseline_checkpoint_dir))
        # frame dirs start with "lora_frame"
        reference_paths = [f"{args.baseline_checkpoint_dir}/{dir}" for dir in reference_paths if dir.startswith("lora_frame")]
        reference_paths = [f"{path}/reference.jpg" for path in reference_paths] # jpg for highway video
        train_video_lora = True
    elif args.image:
        print("single image input")
        train_video_lora = False
        reference_paths = [f"{args.baseline_checkpoint_dir}/reference.png"]
    else:
        assert args.mesh
        print("SDF input")
        train_video_lora = False
        reference_paths = [f"{args.baseline_checkpoint_dir}/reference.obj"]
    ...
    print(args.baseline_checkpoint_dir)
    ...
    for frame_idx, reference_path in enumerate(reference_paths):
        ...
        if args.image or args.video:
            ...
            # SDF data
            assert args.mesh
            sdf_dataset = SDFDataset(reference_path, device=device, clip_sdf=None, num_samples=config["batch_sz"], transformation_save_dir=args.baseline_checkpoint_dir)
            n_channels = 1
            spatial_input_dim = 3
        ...
        base_model_config = json.load(open(f"{args.baseline_checkpoint_dir}/config.json"))
        ...
        base_model_checkpoint = torch.load(f"{args.baseline_checkpoint_dir}/base_model_weights.pt", weights_only=True)
        ...
        # initialize and load baseline MLP
        # mlp.load_state_dict(torch.load(f"{args.baseline_checkpoint_dir}/{save_dirname}/finetuned_weights.pt", weights_only=True)['model_state_dict'])
        forward_fn = lambda input_pos : mlp(encoding(input_pos))
        # prev_frame_checkpoint_paths.append(f"{args.baseline_checkpoint_dir}/{save_dirname}/finetuned_weights.pt")
        mlp.to(device)
        ...
        with open(f"{eval_dir}/{save_dirname}/num_parameters.txt", "w") as file:
            file.write(str(trainable_params))
        if args.image or args.video:
            ...
            assert args.mesh
            save_mesh(save_path, forward_fn, device, resolution=512)
            print("<=== done.")
