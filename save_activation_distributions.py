...
def get_args():
    ...
    parser.add_argument("save_dirname", type=str, help="dirname for dir to be created in activation_distributions/")
...
if __name__ == "__main__":
    ...
    for frame_idx, reference_path in enumerate(reference_paths):
        ...
        ###### save base model linear layer weights
        i=0
        for name,layer in model.named_modules():
            if isinstance(layer, nn.Linear):
                weight = layer.weight.detach().cpu().numpy()
                print(weight.shape)  # [d_out, d_in]
                filepath = f"activation_distributions/{args.save_dirname}/linear_weights/base_layer{i}"
        ...
        assert lora_config["full_finetuning"]
        ...
        ###### save finetuned model linear layer weights
        i=0
        for name,layer in mlp.named_modules():
            if isinstance(layer, nn.Linear):
                weight = layer.weight.detach().cpu().numpy()
                print(weight.shape)  # [d_out, d_in]
                filepath = f"activation_distributions/{args.save_dirname}/linear_weights/finetuned_layer{i}"
                os.makedirs(os.path.dirname(filepath), exist_ok=True)
                np.save(filepath, weight)
                i+= 1
        ...
        if args.image or args.video:
            reconstruction, linear_outputs, activations = forward_fn(xy)
            for i, activation in enumerate(activations):
                print(f"activation layer {i}: {activation.shape}")
                np.save(f"activation_distributions/{args.save_dirname}/finetuned_activations_layer{i}", activation)
# TODO take in dirname and check if exists; save activations there
