...
def get_args():
    ...
    parser.add_argument("svd_weights_dir", type=str, help="e.g. activation_distributions/dof  should be compatible with finetuned_checkpoint_dir")
...
if __name__ == "__main__":
    ...
    eval_dir = f"eval/{args.finetuned_checkpoint_dir}/svd_weighted_postprocess_rank{args.rank}_{timestamp}/"
    ...
    for i in range(7):
        rank_constrained_delta_Ws.append(torch.from_numpy(np.load(f"{args.svd_weights_dir}/opt_deltaW_rank{args.rank}_layer{i}.npy")).to(device))
    ...
    with torch.no_grad():
        ...
        save_path = f"{eval_dir}/post_process_rank{args.rank}_reconstruction.png"
